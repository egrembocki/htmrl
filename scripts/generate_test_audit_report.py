...
def main() -> None:
    ...
    with OUT_CSV.open('w', newline='', encoding='utf-8') as f:
        w = csv.writer(f)
        w.writerow(['file', 'line', 'test_name', 'ts_id', 'tc_id', 'ti_id', 'commit_tag', 'test_type', 'type_source'])
        for r in rows:
            w.writerow([r.file, r.line, r.test_name, r.ts_id, r.tc_id, r.ti_id, r.commit_tag, r.test_type, r.type_source])
    ...
    md.append('')
    md.append('Generated from all `tests/test_*.py` and `tests/integration/test_*.py` functions named `test_*`.')
    md.append('')
    md.append('## Summary')
    md.append('')
    ...
    md.append('|---|---|---|---|---|---|---:|---|')
    for r in rows:
        md.append(f'| {r.test_type} | {r.ts_id} | {r.tc_id} | {r.ti_id} | `{r.test_name}` | `{r.file}` | {r.line} | `{r.type_source}` |')
